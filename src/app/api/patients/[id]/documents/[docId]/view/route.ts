...
// GET - Visualizar documento via proxy seguro
...
export async function GET(
  ...
  try {
    ...
    // 5. Registrar auditoria de visualização (conformidade LGPD)
    await logDocumentEvent("DOCUMENT_VIEW", session.user.id, docId, {
      documentName: document.name,
      patientId,
    }, request)

    // 6. Retornar o arquivo com headers corretos para visualização inline
